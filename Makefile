...
VERSION_TAG := $(shell git describe --tags --abbrev=0 2>/dev/null || echo $(BRANCH)-$(COMMIT))
...
# fallback if no tag use commit branch
build-image:
	docker build -f Swoole.Dockerfile -t $(IMAGE_NAME):$(VERSION_TAG) -t $(IMAGE_NAME):latest .
